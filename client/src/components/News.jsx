    ...
    <div className='conatainer_news'>
      <div className='hindustanNews'>
        ...
        <div className='latest'>
          ...
          {allnews.map((e, i) => {
            if (i == 0 && e.urlToImage !== null) {
              var newsApiDate = e.publishedAt;
              var timeStamp = new Date(newsApiDate).getTime();
              var day = new Date(newsApiDate).getDay();
              var month = new Date(newsApiDate).getMonth() + 1;
              var year = new Date(newsApiDate).getFullYear();
              var dateFormat = `Updated on ${month}-${day}-${year}`;
              dateFormat = dateFormat.toLocaleString("default", {
                month: "long",
              });
              return (
                <>
              ...
              let newsApiDate = e.publishedAt;
              let timeStamp = new Date(newsApiDate).getTime();
              let day = new Date(newsApiDate).getDay();
              let month = new Date(newsApiDate).getMonth() + 1;
              let year = new Date(newsApiDate).getFullYear();
              let dateFormat = `Updated on ${month}-${day}-${year}`;
              dateFormat = dateFormat.toLocaleString("default", {
                month: "long",
              });
                      ...
                      <div className='news-card-left'>
                        ...
                        <div>
                          ...
                          <div className='news-card-bottom-right'>
                            <img
                              className='news-share-img'
                              src={share}
                              alt=''
                            />
                            <img className='news-save-img' src={save} alt='' />
                          </div>
                        </div>
                      </div>
